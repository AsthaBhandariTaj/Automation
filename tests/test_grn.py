...
class TestGrn:
    def test_valid_grn(self, logged_in_driver):
        ...
        stock_in_hand.open()
        time.sleep(5)
        before_stock = stock_in_hand.stock_in_hand(items="liquid soap")
        time.sleep(5)
        quantity = 2
        grn_page = Grn(logged_in_driver)
        grn_page.open()
        grn_page.open()
        grn_page.grn(quantity)
        time.sleep(2)
        ...
        stock_in_hand.open()
        time.sleep(5)
        after_stock = stock_in_hand.stock_in_hand(items="liquid soap")
